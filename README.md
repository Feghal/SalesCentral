...
The SDK never parses an Apple receipt. It uploads the signed JWS
(`VerificationResult.jwsRepresentation`) verbatim and the server does all
validation. Each app gets its own set of 10 unguessable URLs (generated by
the admin); the SDK composes calls from those.
Money-touching calls are further protected by Apple App Attest — see
[App Attest](#app-attest) below.
...
generates a Secure Enclave key, has Apple attest it, and registers it with
the server, then attaches a per-request hardware assertion to money-touching
...
1. Add the App Attest entitlement to your app target
   ...
   for Xcode debug builds, `production` for TestFlight/App Store.
2. Set the app's Team ID + environment on the admin's App Detail page (App
   Attest card) — the attested App ID is `<Team ID>.<bundleId>`.
...
func buy(_ product: Product) async {
    do {
        ...
        case .notEntitled(let reason): showPaywall(reason)  // expired/already-owned — nothing granted
        }
...
`await`. You never call `Transaction.updates`, `txn.finish()`, or extract
the `jwsRepresentation` yourself.
